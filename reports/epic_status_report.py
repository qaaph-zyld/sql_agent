"""
EPIC Status Report Module

This module generates reports on items with EPIC status in BOM.
It identifies items where [Item Number Status] = "EPIC" and [FG/SFG/RM] <> "No BOM".
"""
...
class EpicStatusReport(DataQualityReport):
    """Report on items with EPIC status in BOM"""
    
    def __init__(self, config_manager):
        super().__init__(config_manager)
        self.report_type = "epic_status"
    
    def generate_report(self):
        """Generate report on items with EPIC status in BOM"""
        logger.info("Generating EPIC status report")
        
        # Execute direct query for items with EPIC status in BOM
        query = """
        SELECT 
            ...
            CASE WHEN pt.pt_status = 'EPIC' AND 
                     CASE 
                         WHEN b.Parent = 'Yes' AND b.Child = 'No' THEN 'FG'
                         WHEN b.SFG = 'Yes' THEN 'SFG'
                         WHEN b.Parent = 'No' AND b.Child = 'Yes' THEN 'RM'
        ...
        WHERE 
            ...
            END <> 'No BOM'
        ORDER BY 
            pt.pt_group, pt.pt_prod_line
        """
        
        df = self.execute_query(query)
        logger.info(f"Found {len(df)} items with EPIC status in BOM")
        ...
        logger.info("EPIC status report generation completed")
    ...
    def _generate_charts(self, group_summary, plant_summary, type_summary):
        ...
        plt.title('Top 10 Groups with EPIC Status Items in BOM')
        ...
        plt.title('EPIC Status Items by Plant')
        plt.tight_layout()
        ...
        plt.title('EPIC Status Items by Item Type')
        plt.tight_layout()
    ...
    def _generate_trend_analysis(self):
        ...
        if len(historical_data) > 1:
            ...
            plt.title('Trend of EPIC Status Items in BOM')
            plt.xlabel('Date')
...
if __name__ == "__main__":
    ...
    report = EpicStatusReport(config_manager)
