"""
Operation Check Report Module

This module generates reports on items with operation check issues.
It identifies items with multiple operations but not present in both plants.
"""
...
class OperationCheckReport(DataQualityReport):
    """Report on items with operation check issues"""
    
    def __init__(self, config_manager):
        super().__init__(config_manager)
        self.report_type = "operation_check"
    
    def generate_report(self):
        """Generate report on items with operation check issues"""
        logger.info("Generating operation check report")
        
        # Execute direct query for items with operation check issues
        query = """
        WITH ItemInBothPlants AS (
            SELECT 
                pt1.pt_part
            FROM 
                [QADEE2798].[dbo].[pt_mstr] pt1
            WHERE 
                pt1.pt_site = '2798'
                AND EXISTS (
                    SELECT 1 
                    FROM [QADEE2798].[dbo].[pt_mstr] pt2 
                    WHERE pt2.pt_part = pt1.pt_part AND pt2.pt_site = '2674'
                )
        )
        SELECT 
            ...
            CASE WHEN NOT EXISTS (
                SELECT 1 FROM ItemInBothPlants ibp WHERE ibp.pt_part = pt.pt_part
            ) THEN 'Yes' ELSE 'No' END AS [Operation Check]
        FROM 
        ...
        logger.info(f"Found {len(df)} items with operation check issues")
        ...
        # No longer using operation count in our simplified query
        ...
        logger.info("Operation check report generation completed")
    ...
    def _generate_charts(self, group_summary, plant_summary, type_summary):
        ...
        plt.title('Top 10 Groups with Operation Check Issues')
        ...
        plt.title('Operation Check Issues by Plant')
        plt.tight_layout()
        ...
        plt.title('Operation Check Issues by Item Type')
        plt.tight_layout()
        ...
        # Operation count chart no longer needed
    ...
    def _generate_trend_analysis(self):
        ...
        if len(historical_data) > 1:
            ...
            plt.title('Trend of Items with Operation Check Issues')
            plt.xlabel('Date')
...
if __name__ == "__main__":
    ...
    report = OperationCheckReport(config_manager)
