"""
Missing Cost Report Module

This module generates reports on items with missing cost information.
It identifies items where [Standard Cost] = 0 and [FG/SFG/RM] <> "No BOM".
"""
...
class MissingCostReport(DataQualityReport):
    """Report on items with missing cost information"""
    
    def __init__(self, config_manager):
        super().__init__(config_manager)
        self.report_type = "missing_cost"
    
    def generate_report(self):
        """Generate report on items with missing cost"""
        logger.info("Generating missing cost report")
        
        # Execute direct query for items with missing cost
        query = """
        SELECT 
        ...
        logger.info(f"Found {len(df)} items with missing cost")
        ...
        logger.info("Missing cost report generation completed")
    ...
    def _generate_charts(self, group_summary, plant_summary, type_summary):
        ...
        plt.title('Top 10 Groups with Missing Cost Information')
        ...
        plt.title('Missing Cost Information by Plant')
        plt.tight_layout()
        ...
        plt.title('Missing Cost Information by Item Type')
        plt.tight_layout()
    ...
    def _generate_trend_analysis(self):
        ...
        if len(historical_data) > 1:
            ...
            plt.title('Trend of Items with Missing Cost Information')
            plt.xlabel('Date')
...
if __name__ == "__main__":
    ...
    report = MissingCostReport(config_manager)
