"""
Group Issues Report Module

This module generates reports on items with missing or invalid group information.
It identifies items where [Group] = "F000" and [FG/SFG/RM] <> "No BOM".
"""
...
class GroupIssuesReport(DataQualityReport):
    """Report on items with missing or invalid group information"""
    
    def __init__(self, config_manager):
        super().__init__(config_manager)
        self.report_type = "group_issues"
    
    def generate_report(self):
        """Generate report on items with group issues"""
        logger.info("Generating group issues report")
        
        # Execute direct query for items with group issues
        query = """
        SELECT 
            ...
            CASE WHEN pt.pt_group = 'F000' AND 
                     CASE 
                         WHEN b.Parent = 'Yes' AND b.Child = 'No' THEN 'FG'
                         WHEN b.SFG = 'Yes' THEN 'SFG'
                         WHEN b.Parent = 'No' AND b.Child = 'Yes' THEN 'RM'
        ...
        WHERE 
            ...
            END <> 'No BOM'
        ORDER BY 
            pt.pt_prod_line, pt.pt_part
        """
        
        df = self.execute_query(query)
        logger.info(f"Found {len(df)} items with group issues")
        
        # Generate summary by product line
        prodline_summary = df.groupby(['Prod Line']).size().reset_index(name='Count')
        prodline_summary = prodline_summary.sort_values('Count', ascending=False)
        ...
        prodline_file = f"{self.report_type}_by_prodline_{self.report_date}.xlsx"
        self.export_to_excel(prodline_summary, prodline_file)
        ...
        # Generate charts
        self._generate_charts(prodline_summary, plant_summary, type_summary)
        
        # Save metadata
        metadata = {
            'report_date': self.report_date,
            'total_count': len(df),
            'prodlines_affected': df['Prod Line'].nunique(),
            'plants_affected': df['Plant'].nunique(),
            'types_affected': df['FG/SFG/RM'].nunique(),
            'top_prodline': prodline_summary.iloc[0]['Prod Line'] if not prodline_summary.empty else None,
            'top_prodline_count': int(prodline_summary.iloc[0]['Count']) if not prodline_summary.empty else 0
        }
        ...
        logger.info("Group issues report generation completed")
        
        return {
            'detail_file': detail_file,
            'prodline_file': prodline_file,
            'plant_file': plant_file,
            'type_file': type_file,
            'metadata': metadata
        }
    
    def _generate_charts(self, prodline_summary, plant_summary, type_summary):
        """Generate charts for the report"""
        # Product Line chart
        plt.figure(figsize=(12, 6))
        top_prodlines = prodline_summary.head(10)
        sns.barplot(x='Prod Line', y='Count', data=top_prodlines)
        plt.title('Top 10 Product Lines with Group Issues')
        plt.xticks(rotation=45)
        plt.tight_layout()
        
        prodline_chart = f"{self.report_type}_prodline_chart_{self.report_date}.png"
        plt.savefig(os.path.join(self.output_dir, prodline_chart))
        plt.close()
        
        # Plant chart
        plt.figure(figsize=(10, 6))
        sns.barplot(x='Plant', y='Count', data=plant_summary)
        plt.title('Group Issues by Plant')
        plt.tight_layout()
        ...
        plt.title('Group Issues by Item Type')
        plt.tight_layout()
    ...
    def _generate_trend_analysis(self):
        ...
        if len(historical_data) > 1:
            ...
            plt.title('Trend of Items with Group Issues')
            plt.xlabel('Date')
...
if __name__ == "__main__":
    ...
    report = GroupIssuesReport(config_manager)
